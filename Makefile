...
streampanel-windows: builddir
	CGO_ENABLED=1 CC=x86_64-w64-mingw32-gcc GOOS=windows go build $(GOBUILD_FLAGS) -ldflags "-H windowsgui" -o build/streampanel.exe ./cmd/streampanel/

streampanel-windows-debug: builddir
	CGO_ENABLED=1 CC=x86_64-w64-mingw32-gcc GOOS=windows go build $(GOBUILD_FLAGS) -o build/streampanel-debug.exe ./cmd/streampanel/
