	...
	observability.Go(ctx, func(ctx context.Context) {
		...
		for {
			...
			case ev, ok := <-msgCh:
				...
				if ev.TargetChannel.Slug != k.Channel.Livestream.Slug {
					logger.Warnf(ctx, "skipping a message for another channel: %q != %q", ev.TargetChannel.Slug, k.Channel.Livestream.Slug)
					continue
				}
