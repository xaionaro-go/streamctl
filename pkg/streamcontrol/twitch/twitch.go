...
func (t *Twitch) Shoutout(
	...
	reply, err := t.client.GetStreams(&helix.StreamsParams{
		UserIDs: []string{string(userIDOrLogin)},
	})
	if err != nil {
		reply, err = t.client.GetStreams(&helix.StreamsParams{
			UserLogins: []string{string(userIDOrLogin)},
		})
		if err != nil {
			logger.Errorf(ctx, "unable to get channel info ('%s'): %w", userIDOrLogin, err)
...
func (t *Twitch) sendShoutoutMessageWithoutChanInfo(
	...
	err := t.SendChatMessage(ctx, fmt.Sprintf("Shoutout to %s! A great creator (%f years at Twitch)! Their self-description: '%s'. Take a look at their channel and click that follow button! https://www.twitch.tv/%s", user.DisplayName, yearsExists, user.Description, user.Login))
	if err != nil {
...
func (t *Twitch) sendShoutoutMessage(
	...
	yearsExists := float64(int(time.Since(user.CreatedAt.Time).Hours()/24/364*10)) / 10
	err := t.SendChatMessage(ctx, fmt.Sprintf("Shoutout to %s! A great creator (%f years at Twitch)! Their last stream: '%s'. Their self-description: '%s'. Take a look at their channel and click that follow button! https://www.twitch.tv/%s", user.DisplayName, yearsExists, stream.Title, user.Description, user.Login))
	if err != nil {
