...
// capturedInjectCall records a single InjectPlatformEvent invocation.
...
// Only InjectPlatformEvent is implemented; all other methods panic if called.
...
func (m *mockStreamDClient) InjectPlatformEvent(
	_ context.Context,
	in *streamd_grpc.InjectPlatformEventRequest,
	_ ...grpc.CallOption,
) (*streamd_grpc.InjectPlatformEventReply, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	var eventID string
	if ev := in.GetMessage(); ev != nil {
		eventID = ev.GetId()
	}
...
func TestRunnerSingleListener_RetriesOnFailure(t *testing.T) {
	...
	assert.GreaterOrEqual(t, len(calls), 2,
		"runner must retry: expected at least 2 InjectPlatformEvent calls, got %d", len(calls))
...
func TestRunnerSingleListener_ContextCancellation(t *testing.T) {
	...
	assert.Greater(t, len(calls), 0,
		"expected at least one InjectPlatformEvent call before cancellation")
}
...
func TestRunnerSingleListener_InjectsKeepalive(t *testing.T) {
	...
	calls := client.getCalls()
	require.Greater(t, len(calls), 0, "expected InjectPlatformEvent calls")
