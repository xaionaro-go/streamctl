...
type Client struct {
	// streamd_grpc.ProxyClient is the autogenerated client-side wrapper
	// emitted by protoc-gen-streamd-shim for RPCs annotated with
	// `option (streamd.proxy) = AUTO;`. It dispatches through Backend, which
	// is set to *Client itself (see New / WrapConn) so the wrapper reuses our
	// retry, ProcessError, and call-wrapper plumbing.
	streamd_grpc.ProxyClient
	Stats struct {
		BytesIn  uint64
		BytesOut uint64
	}
	...
	_, err := withStreamDClient(ctx, c, func(
		...
		return callWrapper(
			...
			&streamd_grpc.InjectChatMessageRequest{
				...
				ListenerType: listenerType.String(),
			},
		)
	})
	if err != nil {
...
func (c *Client) ReportChatHandlerActivity(
	ctx context.Context,
	platID streamcontrol.PlatformName,
	listenerType streamcontrol.ChatListenerType,
) {
	_, err := withStreamDClient(ctx, c, func(
		...
		return callWrapper(
			ctx,
			c,
			client.ReportChatHandlerActivity,
			&streamd_grpc.ReportChatHandlerActivityRequest{
				PlatID:       string(platID),
				ListenerType: listenerType.String(),
			},
		)
	...
	if err != nil {
		logger.Warnf(ctx, "ReportChatHandlerActivity failed: %v", err)
	}
}

func (c *Client) ReportTranslatorActivity(
	ctx context.Context,
) {
	_, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.ReportTranslatorActivityReply, error) {
		return callWrapper(
			ctx,
			c,
			client.ReportTranslatorActivity,
			&streamd_grpc.ReportTranslatorActivityRequest{},
		)
	})
	if err != nil {
		logger.Warnf(ctx, "ReportTranslatorActivity failed: %v", err)
	}
}
...
// TranslatorStats satisfies api.StreamD. The wire reply is unmarshalled
// back into the in-process snapshot type so the API contract stays uniform
// across the in-process and remote backends. Callers that need the raw
// proto reply (cmd/streamcli's tabwriter / JSON formatter) should use
// TranslatorStatsRaw.
//
// In-band Error per the Translator* service-level rule on streamd.proto is
// translated into a Go error here, but the partial snapshot is still
// returned so callers that want both can inspect snap.* even when the
// subprocess Stats RPC failed. This matches the "(snap, err)" contract the
// in-process backend exposes. Transport failure (non-nil err returned by
// TranslatorStatsRaw) returns (nil, err); only the in-band-Error path
// preserves partial-data.
func (c *Client) TranslatorStats(
	ctx context.Context,
) (*api.TranslatorStatsSnapshot, error) {
	reply, err := c.TranslatorStatsRaw(ctx)
	if err != nil {
		return nil, err
	}
	snap := &api.TranslatorStatsSnapshot{
		Running:                        reply.GetRunning(),
		PID:                            reply.GetPid(),
		SocketPath:                     reply.GetSocketPath(),
		LastActivityUnixNano:           reply.GetLastActivityUnixNano(),
		QueueLen:                       reply.GetQueueLen(),
		QueueCap:                       reply.GetQueueCap(),
		QueueDrops:                     reply.GetQueueDrops(),
		TotalOffered:                   reply.GetTotalOffered(),
		TotalEnqueued:                  reply.GetTotalEnqueued(),
		TotalResolved:                  reply.GetTotalResolved(),
		QueueFullAtEnqueue:             reply.GetQueueFullAtEnqueue(),
		OfferedWithTranslationDisabled: reply.GetOfferedWithTranslationDisabled(),
	}
	if d := reply.GetDispositions(); d != nil {
		snap.Dispositions = api.TranslationDispositionCounters{
			Translated:                 d.GetTranslated(),
			AlreadyTarget:              d.GetAlreadyTarget(),
			DetectFailed:               d.GetDetectFailed(),
			SpellingOnly:               d.GetSpellingOnly(),
			AllProvidersFailed:         d.GetAllProvidersFailed(),
			ChainSkippedQueueFull:      d.GetChainSkippedQueueFull(),
			AbandonedOnDisable:         d.GetAbandonedOnDisable(),
			AbandonedOnSubprocessDeath: d.GetAbandonedOnSubprocessDeath(),
			AbandonedOnShutdown:        d.GetAbandonedOnShutdown(),
			Flushed:                    d.GetFlushed(),
			Leaked:                     d.GetLeaked(),
			EmptyOrSkipped:             d.GetEmptyOrSkipped(),
		}
	}
	// Reconstruct the StatsReply only when the subprocess clearly
	// answered (non-zero counters or providers). This is best-effort:
	// the wire reply does not carry a "subprocess reachable" bit, so an
	// empty SubprocessStats vs a fully zero one is ambiguous. We pick
	// "non-empty providers" as the reachable signal and otherwise treat
	// it as if the subprocess was not queryable on this hop.
	if len(reply.GetProviders()) > 0 || reply.GetLatencyCount() > 0 ||
		reply.GetTotalTranslated() > 0 || reply.GetTotalAlreadyTarget() > 0 ||
		reply.GetTotalDetectFailed() > 0 || reply.GetTotalSpellingOnly() > 0 ||
		reply.GetTotalAllProvidersFailed() > 0 || reply.GetTotalSkippedQueueFull() > 0 {
		snap.SubprocessStats = &translator_grpc.StatsReply{
			TotalTranslated:         reply.GetTotalTranslated(),
			TotalAlreadyTarget:      reply.GetTotalAlreadyTarget(),
			TotalDetectFailed:       reply.GetTotalDetectFailed(),
			TotalSpellingOnly:       reply.GetTotalSpellingOnly(),
			TotalAllProvidersFailed: reply.GetTotalAllProvidersFailed(),
			TotalSkippedQueueFull:   reply.GetTotalSkippedQueueFull(),
			LatencySumNanos:         reply.GetLatencySumNanos(),
			LatencyCount:            reply.GetLatencyCount(),
		}
		for _, p := range reply.GetProviders() {
			snap.SubprocessStats.Providers = append(snap.SubprocessStats.Providers, &translator_grpc.ProviderStats{
				Name:                 p.GetName(),
				TotalCalls:           p.GetTotalCalls(),
				TotalSuccesses:       p.GetTotalSuccesses(),
				TotalErrors:          p.GetTotalErrors(),
				TotalTimeouts:        p.GetTotalTimeouts(),
				TotalQueueRejections: p.GetTotalQueueRejections(),
				LatencySumNanos:      p.GetLatencySumNanos(),
				LatencyCount:         p.GetLatencyCount(),
			})
		}
	}
...
// TranslatorStatsRaw returns the wire reply unchanged so the cmd-layer can
// format tabular or JSON output without re-deriving fields.
func (c *Client) TranslatorStatsRaw(
	ctx context.Context,
) (*streamd_grpc.TranslatorStatsReply, error) {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
		...
		return callWrapper(
			ctx,
			c,
			client.TranslatorStats,
			&streamd_grpc.TranslatorStatsRequest{},
		)
	})
	if err != nil {
		return nil, fmt.Errorf("unable to query translator stats: %w", err)
	}
	return resp, nil
}

// TranslatorReload satisfies api.StreamD via the embedded
// streamd_grpc.ProxyClient (autogenerated from
// `option (streamd.proxy) = AUTO;` on the proto). Behaviour is unchanged: the
// boolean carries the subprocess `applied` flag, transport errors propagate
// wrapped, and the in-band streamd-level error is folded back into the
// returned error. Callers that need to distinguish "config rejected" from
// "transport down" should use TranslatorReloadDetail below.

// TranslatorReloadDetail mirrors LLMGenerate's "raw reply" shape: the
// in-band streamd-level error is returned as errMsg, while transport
// errors come through err. Used by the cmd/streamcli layer so an operator
// running `translator reload` against a streamd with translation disabled
// gets a precise message instead of a wrapped Go error.
func (c *Client) TranslatorReloadDetail(
	ctx context.Context,
) (applied bool, errMsg string, err error) {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorReloadReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorReload,
			&streamd_grpc.TranslatorReloadRequest{},
		)
	})
	if err != nil {
		return false, "", fmt.Errorf("unable to reload translator: %w", err)
	}
	return resp.GetApplied(), resp.GetError(), nil
}

// TranslatorEnable forwards to streamd's TranslatorEnable RPC. Returns
// an error when transport fails OR the streamd-side handler reports
// started=false (i.e., the in-band error string is wrapped into the Go
// error so the api.StreamD shape stays uniform).
func (c *Client) TranslatorEnable(
	ctx context.Context,
	...
	resp, err := withStreamDClient(ctx, c, func(
		...
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorEnableReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorEnable,
			&streamd_grpc.TranslatorEnableRequest{TargetLanguage: targetLanguage},
		)
	})
	if err != nil {
		return fmt.Errorf("unable to enable translator: %w", err)
	}
	if !resp.GetStarted() {
...
// TranslatorDisable forwards to streamd's TranslatorDisable RPC. The
// in-band error is folded into the returned error so callers see a single
// failure surface.
func (c *Client) TranslatorDisable(
	ctx context.Context,
) error {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorDisableReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorDisable,
			&streamd_grpc.TranslatorDisableRequest{},
		)
	})
	if err != nil {
		return fmt.Errorf("unable to disable translator: %w", err)
	}
...
// TranslatorRestart forwards to streamd's TranslatorRestart RPC.
func (c *Client) TranslatorRestart(
	ctx context.Context,
) error {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorRestartReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorRestart,
			&streamd_grpc.TranslatorRestartRequest{},
		)
	})
	if err != nil {
		return fmt.Errorf("unable to restart translator: %w", err)
	}
...
// TranslatorClearHistory forwards to streamd's TranslatorClearHistory RPC
// and returns the dropped-entries count from the subprocess. The in-band
// error string surfaces as a Go error.
func (c *Client) TranslatorClearHistory(
	ctx context.Context,
) (int32, error) {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorClearHistoryReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorClearHistory,
			&streamd_grpc.TranslatorClearHistoryRequest{},
		)
	})
	if err != nil {
		return 0, fmt.Errorf("unable to clear translator history: %w", err)
	}
	if !resp.GetCleared() {
		return 0, errors.New(resp.GetError())
	}
	return resp.GetDroppedEntries(), nil
}

// TranslatorTranslate forwards an ad-hoc Translate RPC to streamd, which in
// turn calls into the running translator subprocess. The in-band error
// string surfaces as a Go error so the api.StreamD shape stays uniform with
// the other Translator wrappers. Drops backend timings on the floor; use
// TranslatorTranslateWithTimings to surface them.
func (c *Client) TranslatorTranslate(
	ctx context.Context,
	user string,
	message string,
) (string, string, time.Duration, error) {
	result, outcome, latency, _, err := c.TranslatorTranslateWithTimings(ctx, user, message)
...
// streamd subprocess attached to the reply (Ollama populates them; others
// return nil) so streamcli can render a model-load / prompt-eval /
// generation breakdown for slow calls.
func (c *Client) TranslatorTranslateWithTimings(
	ctx context.Context,
	user string,
	message string,
) (string, string, time.Duration, *translator_grpc.TranslateTimings, error) {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorTranslateReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorTranslate,
			&streamd_grpc.TranslatorTranslateRequest{
				User:    user,
				Message: message,
			},
		)
	...
	if err != nil {
		return "", "", 0, nil, fmt.Errorf("unable to translate: %w", err)
	}
	timings := streamdToTranslatorTimings(resp.GetTimings())
	if msg := resp.GetError(); msg != "" {
		return "", "", 0, timings, errors.New(msg)
	}
	return resp.GetResult(),
		resp.GetOutcome(),
		time.Duration(resp.GetLatencyNanos()),
		timings,
		nil
}

// TranslatorTranslateViaProvider forwards a provider-bypass Translate RPC to
// streamd, which in turn calls a single named provider on the subprocess
// chain directly. The in-band error string surfaces as a Go error so the
// api.StreamD shape stays uniform with the other Translator wrappers. Drops
// backend timings; use TranslatorTranslateViaProviderWithTimings to keep them.
func (c *Client) TranslatorTranslateViaProvider(
	ctx context.Context,
	user string,
	message string,
	providerName string,
) (string, time.Duration, error) {
	result, latency, _, err := c.TranslatorTranslateViaProviderWithTimings(ctx, user, message, providerName)
...
// timings.
func (c *Client) TranslatorTranslateViaProviderWithTimings(
	ctx context.Context,
	user string,
	message string,
	providerName string,
) (string, time.Duration, *translator_grpc.TranslateTimings, error) {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorTranslateViaProviderReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorTranslateViaProvider,
			&streamd_grpc.TranslatorTranslateViaProviderRequest{
				User:         user,
				Message:      message,
	...
	if err != nil {
		return "", 0, nil, fmt.Errorf("unable to translate via provider: %w", err)
	}
	timings := streamdToTranslatorTimings(resp.GetTimings())
	if msg := resp.GetError(); msg != "" {
		return "", time.Duration(resp.GetLatencyNanos()), timings, errors.New(msg)
	}
	return resp.GetResult(),
		time.Duration(resp.GetLatencyNanos()),
		timings,
...
// streamdToTranslatorTimings converts the streamd-facing timings sub-message
// to the translator-facing one. The two messages carry the same six fields
// in the same order; the streamd surface mirrors translator's so wire types
// stay parallel between the two services. Returns nil when the streamd
// reply did not attach timings (no chain populated them, or the chain
// skipped before reaching a real Translate). Field-by-field copy preserves
// proto3 optional presence: a nil pointer on either side stays nil on the
// other.
func streamdToTranslatorTimings(in *streamd_grpc.TranslatorTranslateTimings) *translator_grpc.TranslateTimings {
	if in == nil {
		return nil
	}
	return &translator_grpc.TranslateTimings{
		TotalDurationNanos:      in.TotalDurationNanos,
		LoadDurationNanos:       in.LoadDurationNanos,
		PromptEvalDurationNanos: in.PromptEvalDurationNanos,
		EvalDurationNanos:       in.EvalDurationNanos,
		PromptEvalTokens:        in.PromptEvalTokens,
		EvalTokens:              in.EvalTokens,
	}
}

// TranslatorQueueList forwards to streamd's TranslatorQueueList RPC and
// returns the streamd-side queue snapshot in api shape so callers do not
// see the proto wire types.
func (c *Client) TranslatorQueueList(
	ctx context.Context,
) ([]api.TranslationQueueEntry, error) {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorQueueListReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorQueueList,
			&streamd_grpc.TranslatorQueueListRequest{},
		)
	})
	if err != nil {
		return nil, fmt.Errorf("unable to list translator queue: %w", err)
	}
	if msg := resp.GetError(); msg != "" {
		return nil, errors.New(msg)
	}
	entries := resp.GetEntries()
	out := make([]api.TranslationQueueEntry, 0, len(entries))
	for _, e := range entries {
		out = append(out, api.TranslationQueueEntry{
			...
			EnqueuedAt: time.Unix(0, e.GetEnqueuedAtUnixNano()),
		})
	}
	return out, nil
}

// TranslatorQueueFlush forwards to streamd's TranslatorQueueFlush RPC and
// returns the count of dropped entries.
func (c *Client) TranslatorQueueFlush(
	ctx context.Context,
) (int32, error) {
	resp, err := withStreamDClient(ctx, c, func(
		ctx context.Context,
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorQueueFlushReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorQueueFlush,
			&streamd_grpc.TranslatorQueueFlushRequest{},
		)
	})
	if err != nil {
		return 0, fmt.Errorf("unable to flush translator queue: %w", err)
	}
	if msg := resp.GetError(); msg != "" {
...
// TranslatorCompileTranslate forwards to streamd's TranslatorCompileTranslate
// RPC and returns the rendered system + user prompts the subprocess chain
// WOULD emit for a Translate call (no provider invoked). The in-band error
// string surfaces as a Go error so the api.StreamD shape stays uniform with
// the other Translator wrappers.
func (c *Client) TranslatorCompileTranslate(
	ctx context.Context,
	user string,
	message string,
	...
	resp, err := withStreamDClient(ctx, c, func(
		...
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorCompileTranslateReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorCompileTranslate,
			&streamd_grpc.TranslatorCompileTranslateRequest{
				User:    user,
				Message: message,
			},
		)
	...
	if err != nil {
		return "", "", "", "", fmt.Errorf("unable to compile translate request: %w", err)
	}
	if msg := resp.GetError(); msg != "" {
		return "", "", "", "", errors.New(msg)
	}
	return resp.GetSystemPrompt(),
		resp.GetUserPrompt(),
		resp.GetTargetLang(),
		resp.GetHistorySnapshot(),
		nil
}

// TranslatorCompileLanguageDetect forwards to streamd's
// TranslatorCompileLanguageDetect RPC and returns the rendered system +
// user prompts the subprocess chain WOULD emit for the language-detection
// step (no provider invoked). The in-band error string surfaces as a Go
// error so the api.StreamD shape stays uniform with the other Translator
// wrappers.
func (c *Client) TranslatorCompileLanguageDetect(
	ctx context.Context,
	message string,
) (string, string, string, string, error) {
	resp, err := withStreamDClient(ctx, c, func(
		...
		client streamd_grpc.StreamDClient,
		conn io.Closer,
	) (*streamd_grpc.TranslatorCompileLanguageDetectReply, error) {
		return callWrapper(
			ctx,
			c,
			client.TranslatorCompileLanguageDetect,
			&streamd_grpc.TranslatorCompileLanguageDetectRequest{
				Message: message,
			},
		)
	})
	if err != nil {
		return "", "", "", "", fmt.Errorf("unable to compile language detect request: %w", err)
	}
	if msg := resp.GetError(); msg != "" {
		return "", "", "", "", errors.New(msg)
	}
