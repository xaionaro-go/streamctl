...
type GRPCServer struct {
	// streamd_grpc.ProxyServer is the autogenerated server-side proxy emitted
	// by protoc-gen-streamd-shim for RPCs annotated with
	// `option (streamd.proxy) = AUTO;`. It embeds UnimplementedStreamDServer so
	// GRPCServer continues to satisfy StreamDServer for un-proxied RPCs and
	// remains forward-compatible with new RPCs added to the proto. Hand-written
	// methods on *GRPCServer (depth 0) override generated proxy methods (depth
	// 1), which in turn override Unimplemented (depth 2).
	streamd_grpc.ProxyServer
	StreamD               api.StreamD
...
func NewGRPCServer(streamd api.StreamD) *GRPCServer {
	return &GRPCServer{
		// streamd satisfies streamd_grpc.ProxyServerBackend via its
		// api.StreamD super-interface (which includes api.Translator and
		// every other AUTO-eligible sub-interface).
		ProxyServer:      streamd_grpc.ProxyServer{Backend: streamd},
		StreamD:          streamd,
...
func (grpc *GRPCServer) InjectChatMessage(
	...
	// Backward compat: empty listenerType → primary, so old clients keep
	// working with the new server. Non-empty must parse via the canonical
	// stringer; an unknown value is a client bug, not silently coerced.
	listenerType := streamcontrol.ChatListenerPrimary
	if raw := req.GetListenerType(); raw != "" {
		parsed, err := streamcontrol.ChatListenerTypeFromString(raw)
		if err != nil {
			return nil, fmt.Errorf("invalid listenerType %q: %w", raw, err)
		}
		listenerType = parsed
	}
...
func (grpc *GRPCServer) ReportChatHandlerActivity(
	ctx context.Context,
	req *streamd_grpc.ReportChatHandlerActivityRequest,
) (*streamd_grpc.ReportChatHandlerActivityReply, error) {
	lt, err := streamcontrol.ChatListenerTypeFromString(req.GetListenerType())
	if err != nil {
		return nil, fmt.Errorf("invalid listenerType %q: %w", req.GetListenerType(), err)
	}
	grpc.StreamD.ReportChatHandlerActivity(
		ctx,
		streamcontrol.PlatformName(req.GetPlatID()),
		lt,
	)
...
func (grpc *GRPCServer) ReportTranslatorActivity(
	ctx context.Context,
	req *streamd_grpc.ReportTranslatorActivityRequest,
) (*streamd_grpc.ReportTranslatorActivityReply, error) {
	grpc.StreamD.ReportTranslatorActivity(ctx)
	return &streamd_grpc.ReportTranslatorActivityReply{}, nil
}
...
// TranslatorStats marshals the streamd-side TranslatorStatsSnapshot into the
// wire reply. Producing a non-nil reply even on partial-failure (subprocess
// Stats RPC errored but the queue counters are still meaningful) is
// deliberate: the CLI shows whatever data is available and surfaces the
// error via the in-band reply.Error field per the Translator* service-level
// rule on streamd.proto.
func (grpc *GRPCServer) TranslatorStats(
	ctx context.Context,
	req *streamd_grpc.TranslatorStatsRequest,
) (*streamd_grpc.TranslatorStatsReply, error) {
	snap, err := grpc.StreamD.TranslatorStats(ctx)
	if snap == nil {
		// Defensive: a backend returning (nil, nil) would be a bug, but we
		// surface a clean reply with the in-band error rather than
		// panicking on the marshalling below.
	...
	reply := &streamd_grpc.TranslatorStatsReply{
		...
		LastActivityUnixNano:           snap.LastActivityUnixNano,
		QueueLen:                       snap.QueueLen,
		QueueCap:                       snap.QueueCap,
		QueueDrops:                     snap.QueueDrops,
		TotalOffered:                   snap.TotalOffered,
		TotalEnqueued:                  snap.TotalEnqueued,
		TotalResolved:                  snap.TotalResolved,
		QueueFullAtEnqueue:             snap.QueueFullAtEnqueue,
		OfferedWithTranslationDisabled: snap.OfferedWithTranslationDisabled,
		Dispositions: &streamd_grpc.TranslationDispositionCounters{
			Translated:                 snap.Dispositions.Translated,
			AlreadyTarget:              snap.Dispositions.AlreadyTarget,
			DetectFailed:               snap.Dispositions.DetectFailed,
			SpellingOnly:               snap.Dispositions.SpellingOnly,
			AllProvidersFailed:         snap.Dispositions.AllProvidersFailed,
			ChainSkippedQueueFull:      snap.Dispositions.ChainSkippedQueueFull,
			AbandonedOnDisable:         snap.Dispositions.AbandonedOnDisable,
			AbandonedOnSubprocessDeath: snap.Dispositions.AbandonedOnSubprocessDeath,
			AbandonedOnShutdown:        snap.Dispositions.AbandonedOnShutdown,
			Flushed:                    snap.Dispositions.Flushed,
			Leaked:                     snap.Dispositions.Leaked,
			EmptyOrSkipped:             snap.Dispositions.EmptyOrSkipped,
		},
	}
	if sub := snap.SubprocessStats; sub != nil {
		reply.TotalTranslated = sub.GetTotalTranslated()
		reply.TotalAlreadyTarget = sub.GetTotalAlreadyTarget()
		reply.TotalDetectFailed = sub.GetTotalDetectFailed()
		reply.TotalSpellingOnly = sub.GetTotalSpellingOnly()
		reply.TotalAllProvidersFailed = sub.GetTotalAllProvidersFailed()
		reply.TotalSkippedQueueFull = sub.GetTotalSkippedQueueFull()
		reply.LatencySumNanos = sub.GetLatencySumNanos()
		reply.LatencyCount = sub.GetLatencyCount()
		for _, p := range sub.GetProviders() {
			reply.Providers = append(reply.Providers, &streamd_grpc.TranslatorProviderStats{
				Name:                 p.GetName(),
...
// TranslatorReload is provided by the embedded streamd_grpc.ProxyServer (see
// `option (streamd.proxy) = AUTO;` on the streamd.proto definition). The
// generated implementation delegates to api.StreamD.TranslatorReload exactly
// as the prior hand-written body did: applied/err pair folded onto the reply
// (Applied + Error), gRPC error channel reserved for transport-layer
// failures. Phase 2 of task 16 migrated this RPC first because its reply
// shape `{string error; bool applied}` is the simplest case (one scalar +
// error) AUTO supports.

// TranslatorEnable persists target_language and reconciles the subprocess.
// Following the TranslatorReload precedent, the in-band error is surfaced
// on the reply (started=false + Error) rather than as a gRPC error so the
// CLI can distinguish "config rejected" from "transport down".
func (grpc *GRPCServer) TranslatorEnable(
	ctx context.Context,
	req *streamd_grpc.TranslatorEnableRequest,
) (*streamd_grpc.TranslatorEnableReply, error) {
	err := grpc.StreamD.TranslatorEnable(ctx, req.GetTargetLanguage())
	reply := &streamd_grpc.TranslatorEnableReply{Started: err == nil}
	if err != nil {
		reply.Error = err.Error()
	}
	return reply, nil
...
// TranslatorDisable kills the subprocess and clears target_language.
func (grpc *GRPCServer) TranslatorDisable(
	ctx context.Context,
	req *streamd_grpc.TranslatorDisableRequest,
) (*streamd_grpc.TranslatorDisableReply, error) {
	err := grpc.StreamD.TranslatorDisable(ctx)
	reply := &streamd_grpc.TranslatorDisableReply{}
	if err != nil {
		reply.Error = err.Error()
	}
	return reply, nil
...
// preserving config.
func (grpc *GRPCServer) TranslatorRestart(
	ctx context.Context,
	req *streamd_grpc.TranslatorRestartRequest,
) (*streamd_grpc.TranslatorRestartReply, error) {
	err := grpc.StreamD.TranslatorRestart(ctx)
	reply := &streamd_grpc.TranslatorRestartReply{Restarted: err == nil}
	if err != nil {
		reply.Error = err.Error()
	}
	return reply, nil
...
// TranslatorClearHistory drops the active chain's history via the
// subprocess and reports the dropped count back to the CLI.
func (grpc *GRPCServer) TranslatorClearHistory(
	ctx context.Context,
	req *streamd_grpc.TranslatorClearHistoryRequest,
) (*streamd_grpc.TranslatorClearHistoryReply, error) {
	dropped, err := grpc.StreamD.TranslatorClearHistory(ctx)
	reply := &streamd_grpc.TranslatorClearHistoryReply{
		Cleared:        err == nil,
...
// TranslatorTranslate forwards an ad-hoc Translate RPC into the running
// subprocess. The in-band error is surfaced on the reply (Error field) so
// the CLI can distinguish "translator not registered" from a transport
// failure, matching the pattern set by TranslatorReload/ClearHistory.
//
// Calls TranslatorTranslateWithTimings on the api so the per-call backend
// timing breakdown attached by the subprocess (Ollama populates it; others
// return nil) flows through to the streamd reply unchanged.
func (grpc *GRPCServer) TranslatorTranslate(
	ctx context.Context,
	req *streamd_grpc.TranslatorTranslateRequest,
) (*streamd_grpc.TranslatorTranslateReply, error) {
	result, outcome, latency, timings, err := grpc.StreamD.TranslatorTranslateWithTimings(
		ctx, req.GetUser(), req.GetMessage())
	reply := &streamd_grpc.TranslatorTranslateReply{
		Result:       result,
		Outcome:      outcome,
		LatencyNanos: latency.Nanoseconds(),
		Timings:      translatorToStreamdTimings(timings),
	}
	if err != nil {
		reply.Error = err.Error()
	}
...
// TranslatorTranslateViaProvider forwards an ad-hoc provider-bypass Translate
// RPC into the running subprocess. Mirrors TranslatorTranslate's reply
// shape sans Outcome (the subprocess no longer runs the chain decisioning
// when bypassing) and surfaces in-band errors on the Error field so the CLI
// can disambiguate "no subprocess" / "provider not found" / per-call errors
// from a transport failure.
func (grpc *GRPCServer) TranslatorTranslateViaProvider(
	ctx context.Context,
	req *streamd_grpc.TranslatorTranslateViaProviderRequest,
) (*streamd_grpc.TranslatorTranslateViaProviderReply, error) {
	result, latency, timings, err := grpc.StreamD.TranslatorTranslateViaProviderWithTimings(
		ctx, req.GetUser(), req.GetMessage(), req.GetProviderName())
	reply := &streamd_grpc.TranslatorTranslateViaProviderReply{
		Result:       result,
		LatencyNanos: latency.Nanoseconds(),
		Timings:      translatorToStreamdTimings(timings),
	}
...
// translatorToStreamdTimings converts the translator-facing TranslateTimings
// sub-message to its streamd-facing TranslatorTranslateTimings twin. The
// two carry the same six fields in the same order; this thin copy keeps
// the streamd-vs-translator surfaces independent so a future schema change
// on one side doesn't force the other. Returns nil when the input is nil
// so the streamd reply's optional `timings` field stays unset.
func translatorToStreamdTimings(in *translator_grpc.TranslateTimings) *streamd_grpc.TranslatorTranslateTimings {
	if in == nil {
		return nil
	}
	return &streamd_grpc.TranslatorTranslateTimings{
...
// TranslatorQueueList snapshots the streamd-side translation queue and
// returns one TranslatorQueueEntry per pending job. The in-band error is
// reflected on the reply so the CLI can disambiguate transport failures
// from streamd-side errors.
func (grpc *GRPCServer) TranslatorQueueList(
	ctx context.Context,
	req *streamd_grpc.TranslatorQueueListRequest,
) (*streamd_grpc.TranslatorQueueListReply, error) {
	entries, err := grpc.StreamD.TranslatorQueueList(ctx)
	reply := &streamd_grpc.TranslatorQueueListReply{
		Entries: make([]*streamd_grpc.TranslatorQueueEntry, 0, len(entries)),
	}
	for _, e := range entries {
		reply.Entries = append(reply.Entries, &streamd_grpc.TranslatorQueueEntry{
			Id:                 e.ID,
			Platform:           e.Platform,
			User:               e.User,
			Message:            e.Message,
			EnqueuedAtUnixNano: e.EnqueuedAt.UnixNano(),
		})
	}
	if err != nil {
		reply.Error = err.Error()
...
// TranslatorQueueFlush drains every pending translation job and reports
// how many were dropped. Mirrors the pattern set by TranslatorClearHistory.
func (grpc *GRPCServer) TranslatorQueueFlush(
	ctx context.Context,
	req *streamd_grpc.TranslatorQueueFlushRequest,
) (*streamd_grpc.TranslatorQueueFlushReply, error) {
	dropped, err := grpc.StreamD.TranslatorQueueFlush(ctx)
	reply := &streamd_grpc.TranslatorQueueFlushReply{
		DroppedEntries: dropped,
	}
	if err != nil {
		reply.Error = err.Error()
...
// and returns the rendered system + user prompts the chain WOULD emit for a
// Translate call. The in-band error is reflected on the reply per the
// Translator* uniform error rule.
func (grpc *GRPCServer) TranslatorCompileTranslate(
	ctx context.Context,
	req *streamd_grpc.TranslatorCompileTranslateRequest,
) (*streamd_grpc.TranslatorCompileTranslateReply, error) {
	systemPrompt, userPrompt, targetLang, history, err := grpc.StreamD.TranslatorCompileTranslate(
		ctx, req.GetUser(), req.GetMessage())
	reply := &streamd_grpc.TranslatorCompileTranslateReply{
...
// CompileLanguageDetect RPC and returns the rendered system + user prompts
// the chain WOULD emit for the language-detection step. The in-band error
// is reflected on the reply per the Translator* uniform error rule.
func (grpc *GRPCServer) TranslatorCompileLanguageDetect(
	ctx context.Context,
	req *streamd_grpc.TranslatorCompileLanguageDetectRequest,
) (*streamd_grpc.TranslatorCompileLanguageDetectReply, error) {
	systemPrompt, userPrompt, targetLang, history, err := grpc.StreamD.TranslatorCompileLanguageDetect(
		ctx, req.GetMessage())
	reply := &streamd_grpc.TranslatorCompileLanguageDetectReply{
		SystemPrompt:    systemPrompt,
		UserPrompt:      userPrompt,
		TargetLang:      targetLang,
		HistorySnapshot: history,
